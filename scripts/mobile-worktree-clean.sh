...
# (`co.aitaco.buzz` on iOS, `xyz.block.buzz.mobile` on Android) are never
# touched. Only identifiers under the iOS `co.aitaco.buzz.wt.` worktree
# namespace or with a suffix after the Android production id are matched.
# Run `just mobile-clean` (or this script directly); pass --dry-run to list
# what would be removed without uninstalling.
set -euo pipefail

ios_prefix="co.aitaco.buzz.wt."
