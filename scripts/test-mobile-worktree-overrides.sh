...
grep -q '^BUNDLE_IDENTIFIER = co\.aitaco\.buzz\.wt\.feature-work-1$' "$ios" \
...
grep -q '^BUNDLE_IDENTIFIER = co\.aitaco\.buzz$' "$debug_xcconfig" \
  && pass "Debug.xcconfig defaults to the app bundle identifier" \
  || fail "Debug.xcconfig must default to co.aitaco.buzz"
...
grep -q '^ios_prefix="co.aitaco.buzz.wt\."$' "$clean_script" \
  && pass "cleanup targets the iOS worktree prefix" \
  || fail "cleanup must share the iOS worktree prefix used by worktree overrides"
