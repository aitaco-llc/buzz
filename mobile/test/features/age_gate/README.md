...
- Community tests: confirmed restriction suppresses local presentation without
  changing saved notification preferences or revoking remote leases; a fresh
  launch restores the persisted preferences.
- Swift and Kotlin adapter tests: platform-specific bounds are normalized
  conservatively, including contradictory ranges and extreme integers.
- Swift notification tests: missing, corrupt, and inaccessible fence storage
  are not age-restriction evidence.
