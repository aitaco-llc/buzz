...
test("aitaco shared compute names the empty state and next action", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error("no aitaco shared compute serving members are available"),
    "relay-mesh",
  );
...
test("aitaco shared compute distinguishes relay lookup failures", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error("aitaco shared compute model discovery failed: relay offline"),
    "relay-mesh",
  );
...
test("aitaco shared compute names a missing relay member roster", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error("aitaco shared compute is waiting for the current member roster"),
    "relay-mesh",
  );
