...
test.describe("Doctor panel state screenshots", () => {
  ...
  test("00-runtime-card-layout", async ({ page }) => {
    ...
    await expect(page.getByTestId("doctor-runtime-codex")).not.toContainText(
      "aitaco talks to Codex through the Codex CLI.",
    );
        ...
        },
        {
          ...
          steps: [
            ...
            },
            {
              ...
              stdout: "",
              stderr:
                "The installer finished, but aitaco still could not use codex (observed: NotInstalled).",
              exit_code: null,
              hint: "aitaco requires the vendor CLI executable, not only its desktop app. If the CLI was installed while aitaco was open, restart aitaco and check again.",
            },
          ],
        },
      ],
  ...
  test("11-outdated-adapter-warning", async ({ page }) => {
    ...
    await expect(dialog).toContainText(
      "Older aitaco releases using the legacy adapter may lose community access",
    );
