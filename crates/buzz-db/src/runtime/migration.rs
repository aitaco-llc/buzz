...
mod postgres_tests {
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        assert!(include_str!("../../../../schema/schema.sql")
            .contains("kind IN (1059, 30179, 30300, 30350, 30622, 44100, 44101, 44200, 44201)"));

        // NIP-AR kind:44201 FTS exclusion (0047): same wrap shape as 0014/0033,
        // but guarded — a database whose expression already suppresses 44201
        // (the 0008 fresh-install allowlist, or the out-of-band maintenance
        // rewrite) must not pay a heap rewrite for a no-op. The receipt is
        // public; the exclusion is about search quality, not secrecy.
        assert_eq!(migrations[46].version, 47);
        assert!(migrations[46].sql.as_str().contains("kind = 44201"));
        assert!(migrations[46].sql.as_str().contains("search_tsv"));
        assert!(migrations[46].sql.as_str().contains("already_suppressed"));
